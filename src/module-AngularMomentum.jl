...
function Wigner_dmatrix(jj, mmp, mm, beta::Float64)
    ...
    #
    factor = sqrt( factorial(Int(j+mp)) * factorial(Int(j-mp)) * factorial(Int(j+m)) * factorial(Int(j-m)) )
    wa = 0.
    for  s = 0:100
        if  j+m-s < 0  ||  j-mp-s < 0   break       end
        if  mp - m + s < 0              continue    end
        wa = wa + (-1)^(mp-m+s) * cos(beta/2.)^(2j+m-mp-2s) * sin(beta/2.)^(mp-m+2s) /
                    ( factorial( Int(j+m-s)) * factorial( Int(s)) * factorial(Int(mp-m+s)) * factorial(Int(j-mp-s)) )
    end

    result = factor * wa
    return abs(result) < 1e-15 ? 0.0 : result  # Round to zero if very small
end
